...
def dro_json_to_oks(jsonfile, oksfile, source_id_offset, nomap, lcores):
    ...
    for entry in jsonmap:
        ...
        if entry["kind"] == "eth":
            ...
            if last_eth_pars != None:
                ...
                if pars["rx_mac"] != last_eth_pars["rx_mac"]:
                    ...
                    nic_dal = dal.NICInterface(nic_name,
                    ...
                    rogroup_dal = dal.ReadoutGroup(f"group-{last_eth_source_id}",
                                                   contains=[nic_dal])
                    db.update_dal(rogroup_dal)
                    groups.append(rogroup_dal)
                    eth_streams = []
                    rx_queue = 0
            stream_pars = dal.EthStreamParameters(f"pars-{source_id}",
            ...
            if not last_felix_pars == None:
                if pars["card"] !=  last_felix_pars["card"] or pars["slr"] != last_felix_pars["slr"]:
                    print(f'Adding FelixInterface felix-{last_felix_source_id} slr={last_felix_pars["slr"]}')
                    felix_dal = dal.FelixInterface(f"felix-{last_felix_source_id}",
                                                   card = last_felix_pars["card"],
                                                   slr = last_felix_pars["slr"],
                                                   contains = flx_streams
                                                   )
                    db.update_dal(felix_dal)
                    rogroup_dal = dal.ReadoutGroup(f"group-{last_felix_source_id}",
                                                   contains=[felix_dal])
                    db.update_dal(rogroup_dal)
                    groups.append(rogroup_dal)
                    flx_streams = []
            stream_pars = dal.FelixStreamParameters(f"flxpars-{source_id}",
                                                    protocol = pars["protocol"],
                                                    mode = pars["mode"],
                                                    link = pars["link"]
                                                    )
        ...
        stream = dal.DROStreamConf(f"DROStream-{source_id}",
    ...
    if eth_streams_found:
        nic_name = f"nic-{last_eth_pars['rx_host']}"
        print(f"Ending by adding nic {last_eth_pars['rx_mac']} with id {nic_name}")
        nic_dal = dal.NICInterface(nic_name,
                                   rx_hostname = last_eth_pars["rx_host"],
                                   rx_mac = last_eth_pars["rx_mac"],
                                   rx_ip = last_eth_pars["rx_ip"],
        ...
        nic_dals[nic_name] = nic_dal
        db.update_dal(nic_dal)
        rogroup_dal = dal.ReadoutGroup(f"group-{eth_source_id}",
                                       contains=[nic_dal])
        db.update_dal(rogroup_dal)
        groups.append(rogroup_dal)
            ...
            if entry["kind"] == "eth":
                ...
                if last_pars != None:
                    if pars["tx_host"] != last_pars['tx_host']:
                        ...
                        hermes_controller_dal = dal.HermesController(hermes_id,
                ...
                if pars["tx_mac"] != last_tx_mac:
                    link_dal = dal.HermesLinkConf(hermes_link_id,
                                                  link_id = link_number,
    ...
    if flx_streams_found and len(flx_streams) > 0:
        ...
        felix_dal = dal.FelixInterface(f"felix-{flx_source_id}",
                                       card = last_felix_pars["card"],
                                       slr = last_felix_pars["slr"],
                                       contains = flx_streams
                                       )
        db.update_dal(felix_dal)
        rogroup_dal = dal.ReadoutGroup(f"group-{flx_source_id}",
                                       contains=[felix_dal])
        db.update_dal(rogroup_dal)
        groups.append(rogroup_dal)
